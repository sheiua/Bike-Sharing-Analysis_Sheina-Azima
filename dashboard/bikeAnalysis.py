...
# Jalur ke file CSV
file_path = "day.csv"  # Sesuaikan jalur sesuai dengan kebutuhan

# Membaca file CSV dengan penanganan kesalahan
try:
    data_day = pd.read_csv(file_path)
except FileNotFoundError:
    st.error("File day.csv tidak ditemukan. Pastikan jalur file sudah benar.")
    st.stop()  # Menghentikan eksekusi jika file tidak ditemukan

try:
    data_hour = pd.read_csv("hour.csv")  # Sesuaikan jalur untuk hour.csv
except FileNotFoundError:
    st.error("File hour.csv tidak ditemukan. Pastikan jalur file sudah benar.")
    st.stop()  # Menghentikan eksekusi jika file tidak ditemukan
